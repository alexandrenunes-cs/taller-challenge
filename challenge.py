...
class User:
    ...
    def retrieve_feed(self):
        ...
        for payment in PAYMENTS:
            if self.username in (payment.actor.username, payment.target.username):
                feed.append(payment)
        return feed
    ...
    def pay_with_balance(self, target, amount, note):
        ...
        target.add_to_balance(amount)
        payment = Payment(amount, self, target, note)
        PAYMENTS[payment.id].append(payment)
        return payment
...
class MiniVenmo:
    def create_user(self, username, balance, credit_card_number):
        try:        
            ...
            print("Error: User could not be created", e)
    ...
    def render_feed(self, feed):
        ...
        for payment in feed:
            ...
            feed = f'{actor.username} paid {target.usename} for {payment.note}'
            if actor.is_friend(target.username):
                feed += 'are friends'
